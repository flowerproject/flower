...
private Properties prop = new Properties();
	...
	public int insertBusiness(Connection con, Business b) {
		...
		try {
			...
			pstmt.setString(6, b.getB_address());
	...
	public Business selectOne(Connection con, Business b) {
		...
		try {
			...
			String sql  = prop.getProperty("selectOneBusiness");
			...
			if(rset.next()) {
				...
				// --- 원래 알고 있던 정보
				result.setB_id(b.getB_id());
				result.setB_password(b.getB_password());
				
				// --- DB로부터 가져온 정보
				
				result.setB_no(rset.getInt("b_no"));
				result.setB_name(rset.getString("b_name"));
				result.setB_shopname(rset.getString("b_shopname"));
				result.setB_num(rset.getString("b_num"));
				result.setB_address(rset.getString("b_address"));
				result.setB_ceoname(rset.getString("b_ceoname"));
				result.setB_incharge(rset.getString("b_incharge"));
				result.setB_incharge_p(rset.getString("b_incharge_p"));
				result.setB_email(rset.getString("b_email"));
				result.setB_date(rset.getDate("b_date"));
			}
